...
function usageZh(p) {
  return docShell({ eyebrow: `使用文档 · v${version}`, title: "从安装到第一次注入", lead: "这份指南覆盖当前已验收的 macOS + Codex 路径，以及模块导入、运行状态与常见问题。", toc: [["install","安装"],["product","目标应用"],["enable","启用模块"],["launch","启动与重启"],["status","理解状态"],["agent","Agent 集成"],["import","导入模块"],["data","文件与数据"],["update","软件更新"],["troubleshooting","故障排查"]], body: `
    ...
    <section id="agent"><h2>Taskboard Agent 集成</h2><p>任务看板运行时会接入 <code>manage-taskboard</code> Skill，并把 <code>taskctl</code> 命令链接到 <code>~/.local/bin</code>。新终端可直接运行 <code>taskctl context current --json</code>；关闭任务看板模块后，这些接入会被移除。</p><p>注入器不会覆盖已有同名 Skill 或命令。发生冲突时会在模块状态中显示错误。</p></section>
    ...
    <section id="data"><h2>模块文件与数据</h2><p>导入模块安装在 <code>~/Library/Application Support/dev.cdp-injector.desktop/Modules/&lt;模块 ID&gt;/&lt;版本&gt;/</code>。模块运行数据单独保存在 <code>Module Data/&lt;模块 ID&gt;/</code>；内置 Taskboard 的程序文件位于应用包的 <code>Contents/Resources/builtin-modules/taskboard/</code>。</p><div class="callout warning"><strong>删除应用不会自动删除数据</strong><p>把 CDP注入器拖到废纸篓后，Application Support、缓存以及可能残留的 Agent 接入仍需单独清理。当前版本尚未提供模块卸载和“一并删除所有数据”的界面。</p></div></section>
    <section id="update"><h2>软件更新</h2><p>在<strong>设置 → 软件更新</strong>中点击“检查更新”。发现新版本后，点击“更新并重新启动”；注入器会验证更新签名、下载安装包，停止模块服务并重新启动。</p><div class="callout"><strong>首次启用说明</strong><p>v0.1.2 及更早版本还没有应用内更新能力，需要手动安装 v0.1.3。此后可直接在设置页更新。</p></div></section>
...
function usageEn(p) {
  return docShell({ eyebrow: `User guide · v${version}`, title: "From installation to your first injection", lead: "The current verified flow covers macOS + Codex, local module imports, runtime status, and troubleshooting.", toc: [["install","Install"],["product","Target app"],["enable","Enable modules"],["launch","Launch modes"],["status","Runtime status"],["agent","Agent integration"],["import","Import a module"],["data","Files and data"],["update","Software update"],["troubleshooting","Troubleshooting"]], body: `
    ...
    <section id="agent"><h2>Taskboard Agent integration</h2><p>While Taskboard runs, it mounts the <code>manage-taskboard</code> Skill and links the <code>taskctl</code> command into <code>~/.local/bin</code>. A new terminal can run <code>taskctl context current --json</code>. Disabling Taskboard removes these integrations.</p><p>Injector never overwrites an existing Skill or command with the same name; conflicts appear as module errors.</p></section>
    ...
    <section id="data"><h2>Module files and data</h2><p>Imported modules are installed under <code>~/Library/Application Support/dev.cdp-injector.desktop/Modules/&lt;module-id&gt;/&lt;version&gt;/</code>. Runtime data is stored separately under <code>Module Data/&lt;module-id&gt;/</code>. The built-in Taskboard files live inside the app at <code>Contents/Resources/builtin-modules/taskboard/</code>.</p><div class="callout warning"><strong>Deleting the app does not remove its data</strong><p>Moving CDP Injector to Trash leaves Application Support, caches, and potentially active Agent integrations for manual cleanup. This version does not yet include module uninstall or a remove-all-data action.</p></div></section>
    <section id="update"><h2>Software update</h2><p>Open <strong>Settings → Software update</strong> and select “Check for updates”. When a version is available, choose “Update and restart”; Injector verifies its signature, downloads it, stops module services, and restarts.</p><div class="callout"><strong>First supported version</strong><p>v0.1.2 and earlier cannot update themselves. Install v0.1.3 manually once, then use in-app updates.</p></div></section>
...
function downloadPage(lang) {
  ...
  const labels = zh ? {
    ...
    verified: "已验收", preview: "兼容适配中", open: "在 GitHub 下载", note: "发布说明", noteBody: "v0.1.3 起可在设置页检查并安装签名更新。macOS 当前仍使用 ad-hoc 应用签名；首次打开如遇系统拦截，请在系统设置的“隐私与安全性”中确认。",
  } : {
    ...
    verified: "Verified", preview: "Compatibility in progress", open: "Download on GitHub", note: "Release note", noteBody: "Starting with v0.1.3, signed updates can be checked and installed from Settings. macOS app bundles still use ad-hoc signing; if first launch is blocked, confirm it in Privacy & Security settings.",
  };
  const cards = [
